...
/*
The following functions have been created and/or modified by Taylor Trabun
in order to be called as an extension in python
*/
static PyMethodDef module_methods[] = {
    {"bdaddr", bdaddr_bdaddr, METH_VARARGS, "Execute bdaddr to spoof BD_ADDR"},
    {NULL, NULL}

};


PyMODINIT_FUNC initbdaddr(void)
{
    (void) Py_InitModule("bdaddr", module_methods);
...
static PyObject *bdaddr_bdaddr(PyObject  *self, PyObject *args)
{
    ...
    log = (char*)malloc(100*sizeof(char));
    if (logging == NULL){
        ...
        string = Py_BuildValue("s", "Logging enabled.");
        PyObject_CallMethod(logging, "debug", "O", string);
    }
    string = Py_BuildValue("s", "About to parse args.");
    PyObject_CallMethod(logging, "debug", "O", string);

	if (!PyArg_ParseTuple(args, "ss", &btDev, &newAddr)){
	    //snprintf(log, 100,"Failed argument parsing. args");
        string = Py_BuildValue("s", "Failed to parse args.");
	    PyObject_CallMethod(logging, "debug", "O", string);
	    return Py_BuildValue("i", -1);
	}

    string = Py_BuildValue("s", "Finished parsing args");
    PyObject_CallMethod(logging, "debug", "O", string);

    dev = hci_devid(btDev);
    snprintf(log, 100, "Retrieved dev device: %i", dev);
    string = Py_BuildValue("s", log);
    PyObject_CallMethod(logging, "debug", "O", string);
    if(dev < 0){
      string = Py_BuildValue("s", "Invalid HCI device");
      PyObject_CallMethod(logging, "debug", "O", string);
      return Py_BuildValue("i", -1);
    }
	...
	string = Py_BuildValue("s", "Attempting to open HCI Device.");
    PyObject_CallMethod(logging, "debug", "O", string);

	dd = hci_open_dev(dev);

	string = Py_BuildValue("s", "Attempted to open HCI Device.");
    PyObject_CallMethod(logging, "debug", "O", string);
	if (dd < 0) {
		//fprintf(stderr, "Can't open device hci%d: %s (%d)\n",
						//dev, strerror(errno), errno);

	    PyErr_Format(PyExc_RuntimeError, "Can't open device hci%d: %s (%d)", dev, strerror(errno), errno);
		return Py_BuildValue("i", -1);
	}
    string = Py_BuildValue("s", "Attempting to read info.");
    PyObject_CallMethod(logging, "debug", "O", string);
	if (hci_devinfo(dev, &di) < 0) {
		//fprintf(stderr, "Can't get device info for hci%d: %s (%d)\n",
						//dev, strerror(errno), errno);
		snprintf(log, 100, "Can't get device info for hci%d: %s (%d)",
						dev, strerror(errno), errno);
        string = Py_BuildValue("s", log);
        PyObject_CallMethod(logging, "debug", "O", string);
		PyErr_Format(PyExc_RuntimeError, "Can't get device info for hci%d: %s (%d)",
						dev, strerror(errno), errno);
		hci_close_dev(dd);
		return Py_BuildValue("i", -1);
	}
    string = Py_BuildValue("s", "Attempting to read version.");
    PyObject_CallMethod(logging, "debug", "O", string);
	if (hci_read_local_version(dd, &ver, 1000) < 0) {
		//fprintf(stderr, "Can't read version info for hci%d: %s (%d)\n",
						//dev, strerror(errno), errno);

		PyErr_Format(PyExc_RuntimeError, "Can't read version info for hci%d: %s (%d)",
						dev, strerror(errno), errno);
		hci_close_dev(dd);
		return Py_BuildValue("i", -1);
	}
    string = Py_BuildValue("s", "Attempting to read BDADDR.");
    PyObject_CallMethod(logging, "debug", "O", string);
	if (!bacmp(&di.bdaddr, BDADDR_ANY)) {
		if (hci_read_bd_addr(dd, &bdaddr, 1000) < 0) {
		    ...
		    PyErr_Format(PyExc_RuntimeError, "Can't read address for hci%d: %s (%d)",
						dev, strerror(errno), errno);
			hci_close_dev(dd);
		    return Py_BuildValue("i", -1);
		}
    ...
    string = Py_BuildValue("s", "Finished gathering info.");
    PyObject_CallMethod(logging, "debug", "O", string);
	//printf("Manufacturer:   %s (%d)\n",
			//bt_compidtostr(ver.manufacturer), ver.manufacturer);
	snprintf(log, 50,"Manufacturer: %s (%d)", bt_compidtostr(ver.manufacturer), ver.manufacturer);
    string = Py_BuildValue("s", log);
	PyObject_CallMethod(logging, "debug", "O", string);

	comp = batocomp(&bdaddr);

    string = Py_BuildValue("s", "Finished batocomp.");
    PyObject_CallMethod(logging, "debug", "O", string);

	ba2str(&bdaddr, addr);

	string = Py_BuildValue("s", "Finished ba2str.");
    PyObject_CallMethod(logging, "debug", "O", string);

	//printf("Device address: %s", addr);
	snprintf(log, 100, "Device address: %s", addr);
	string = Py_BuildValue("s", log);
    PyObject_CallMethod(logging, "debug", "O", string);
	...
	for (i = 0; vendor[i].compid != 65535; i++)
		if (ver.manufacturer == vendor[i].compid) {
			...
			if (vendor[i].write_bd_addr(dd, &bdaddr) < 0) {
				...
				PyErr_SetString(PyExc_RuntimeError, "Can't write new address");
				hci_close_dev(dd);
				return Py_BuildValue("i", -1);
	...
	//printf("\n");
	PyErr_Format(PyExc_RuntimeError, "Unsupported manufacturer");
	return Py_BuildValue("i", -1);
}
